...
export class WorkersService {
  ...
  public getWorkerById(id: number): Observable<Workers> {
    return this.http.get<Workers>(
      GlobalVariables.apiURL + this.workersURL + '/' + id
    );
  }

  public postWorker(worker: Workers): Observable<Workers> {
    return this.http.post<Workers>(
      GlobalVariables.apiURL + this.workersURL,
      worker
    );
  }

  public putWorker(worker: Workers): Observable<Workers> {
    return this.http.put<Workers>(
      GlobalVariables.apiURL + this.workersURL + '/' + worker.workerId,
      worker
    );
  }
}
